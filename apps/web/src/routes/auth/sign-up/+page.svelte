<script lang="ts">
	...
	const signInHref = $derived(`/auth/sign-in?redirectTo=${encodeURIComponent(data.redirectTo)}`);
</script>

<svelte:head><title>Sign Up — History Timeline</title></svelte:head>

<div class="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
	<div class="w-full max-w-sm">
		<h1 class="text-xl font-semibold text-white mb-8 text-center">Sign Up</h1>
		...
		>
			...
			<div class="flex flex-col gap-1.5">
				<label for="name" class="text-xs text-white/50 uppercase tracking-wider">Name</label>
				<input
					id="name"
					name="name"
					type="text"
		...
		<p class="mt-6 text-center text-sm text-white/50">
			...
			<a href={signInHref} class="text-white underline underline-offset-2">Sign In</a>
		</p>
	</div>
</div>
